...
public class Rfc822Output {
     ...
     */
    /*package*/ static String[] buildBodyText(EmailContent.Body body, boolean useSmartReply) {
        if (body == null) {
            return new String[2];
        }
     ...
     */
    public static void writeTo(Context context, EmailContent.Message message, OutputStream out,
            boolean useSmartReply, boolean sendBcc, List<EmailContent.Attachment> attachments)
        ...
        EmailContent.Body body = EmailContent.Body.restoreBodyWithMessageId(context, message.mId);
        ...
        if (attachments == null) {
            attachments =
                    Arrays.asList(EmailContent.Attachment.restoreAttachmentsWithMessageId(context, message.mId));
        }
        ...
        if (!multipart) {
            ...
            if (attachments.size() == 1) {
                ...
                if ((flags & EmailContent.Attachment.FLAG_ICS_ALTERNATIVE_PART) != 0) {
            ...
            for (EmailContent.Attachment att: attachments) {
                writeBoundary(writer, multipartBoundary, false);
