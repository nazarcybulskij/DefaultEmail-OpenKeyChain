...
import com.indeema.emailcommon.provider.MailboxUtilities;
import com.indeema.emailcommon.provider.MessageChangeLogTable;
import com.indeema.emailcommon.provider.MessageMove;
import com.indeema.emailcommon.provider.MessageStateChange;
import com.indeema.emailcommon.provider.Policy;
import com.indeema.emailcommon.provider.QuickResponse;
import com.indeema.emailcommon.service.EmailServiceProxy;
...
import com.indeema.mail.providers.FolderList;
import com.indeema.mail.providers.MessageInfo;
import com.indeema.mail.providers.UIProvider;
import com.indeema.mail.providers.UIProvider.AccountCapabilities;
import com.indeema.mail.providers.UIProvider.AccountCursorExtraKeys;
import com.indeema.mail.providers.UIProvider.ConversationColumns;
import com.indeema.mail.providers.UIProvider.ConversationPriority;
import com.indeema.mail.providers.UIProvider.ConversationSendingState;
import com.indeema.mail.providers.UIProvider.DraftType;
...
import com.indeema.mail.widget.BaseWidgetProvider;
...
public class EmailProvider extends ContentProvider {
    ...
    private static final String[] ORPHANS_PROJECTION
        = new String[] {EmailContent.MessageColumns.ID, EmailContent.MessageColumns.MAILBOX_KEY};
    ...
    static {
        ...
        array.put(MESSAGE_BASE >> BASE_SHIFT, EmailContent.Message.TABLE_NAME);
        array.put(ATTACHMENT_BASE >> BASE_SHIFT, EmailContent.Attachment.TABLE_NAME);
        array.put(HOSTAUTH_BASE >> BASE_SHIFT, HostAuth.TABLE_NAME);
        array.put(UPDATED_MESSAGE_BASE >> BASE_SHIFT, EmailContent.Message.UPDATED_TABLE_NAME);
        array.put(DELETED_MESSAGE_BASE >> BASE_SHIFT, EmailContent.Message.DELETED_TABLE_NAME);
        ...
        array.put(BODY_BASE >> BASE_SHIFT, EmailContent.Body.TABLE_NAME);
        TABLE_NAMES = array;
    }
    ...
    private static final String UPDATED_MESSAGE_INSERT = "insert or ignore into " +
        EmailContent.Message.UPDATED_TABLE_NAME + " select * from " + EmailContent.Message.TABLE_NAME + " where " +
        EmailContent.RECORD_ID + '=';

    private static final String UPDATED_MESSAGE_DELETE = "delete from " +
        EmailContent.Message.UPDATED_TABLE_NAME + " where " + EmailContent.RECORD_ID + '=';

    private static final String DELETED_MESSAGE_INSERT = "insert or replace into " +
        EmailContent.Message.DELETED_TABLE_NAME + " select * from " + EmailContent.Message.TABLE_NAME + " where " +
        EmailContent.RECORD_ID + '=';

    private static final String DELETE_ORPHAN_BODIES = "delete from " + EmailContent.Body.TABLE_NAME +
        " where " + EmailContent.BodyColumns.MESSAGE_KEY + " in " + "(select " + EmailContent.BodyColumns.MESSAGE_KEY +
        " from " + EmailContent.Body.TABLE_NAME + " except select " + EmailContent.RECORD_ID + " from " +
        EmailContent.Message.TABLE_NAME + ')';

    private static final String DELETE_BODY = "delete from " + EmailContent.Body.TABLE_NAME +
        " where " + EmailContent.BodyColumns.MESSAGE_KEY + '=';
    ...
    private void fixParentKeys(SQLiteDatabase db) {
        ...
        db.execSQL("update " + Mailbox.TABLE_NAME + " set " + EmailContent.MailboxColumns.PARENT_KEY + "="
                + "(select " + Mailbox.ID + " from " + Mailbox.TABLE_NAME + " as b where "
                + Mailbox.TABLE_NAME + "." + EmailContent.MailboxColumns.PARENT_SERVER_ID + "="
                + "b." + EmailContent.MailboxColumns.SERVER_ID + " and "
                + Mailbox.TABLE_NAME + "." + EmailContent.MailboxColumns.PARENT_SERVER_ID + " not null and "
                + Mailbox.TABLE_NAME + "." + EmailContent.MailboxColumns.ACCOUNT_KEY
        ...
        db.execSQL("update " + Mailbox.TABLE_NAME + " set " + EmailContent.MailboxColumns.PARENT_KEY
                + "=" + Mailbox.NO_MAILBOX + " where " + EmailContent.MailboxColumns.PARENT_KEY
                + "=" + Mailbox.PARENT_KEY_UNINITIALIZED + " or " + EmailContent.MailboxColumns.PARENT_KEY
                + " is null");
    ...
    public SQLiteDatabase getDatabase(Context context) {
        synchronized (sDatabaseLock) {
            ...
            deleteMessageOrphans(mDatabase, EmailContent.Message.UPDATED_TABLE_NAME);
            deleteMessageOrphans(mDatabase, EmailContent.Message.DELETED_TABLE_NAME);
            // Delete orphaned mailboxes/messages/policies (account no longer exists)
            deleteUnlinked(mDatabase, Mailbox.TABLE_NAME, EmailContent.MailboxColumns.ACCOUNT_KEY,
                    EmailContent.AccountColumns.ID, Account.TABLE_NAME);
            deleteUnlinked(mDatabase, EmailContent.Message.TABLE_NAME, EmailContent.MessageColumns.ACCOUNT_KEY,
                    EmailContent.AccountColumns.ID, Account.TABLE_NAME);
            deleteUnlinked(mDatabase, Policy.TABLE_NAME, EmailContent.PolicyColumns.ID,
                    EmailContent.AccountColumns.POLICY_KEY, Account.TABLE_NAME);
    ...
    private void initUiProvider() {
        ...
        mDatabase.execSQL("update " + Mailbox.TABLE_NAME + " set " + EmailContent.MailboxColumns.UI_SYNC_STATUS +
        ...
        try {
            ...
            switch (match) {
                ...
                case MESSAGE_SELECTION:
                    Cursor findCursor = db.query(tableName, EmailContent.Message.ID_COLUMN_PROJECTION, selection,
                            selectionArgs, null, null, null);
                    try {
                        if (findCursor.moveToFirst()) {
                            return delete(ContentUris.withAppendedId(
                                    EmailContent.Message.CONTENT_URI,
                                    findCursor.getLong(EmailContent.Message.ID_COLUMNS_ID_COLUMN)),
                                    null, null);
                        } else {
                            return 0;
                        }
            ...
            switch (match) {
                ...
                case ATTACHMENTS_MESSAGE_ID:
                    ...
                    result = db.delete(tableName,
                            whereWith(EmailContent.Attachment.MESSAGE_KEY + "=" + id, selection), selectionArgs);
                    break;
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (match == MAILBOX_ID || match == MAILBOX) {
            values.put(EmailContent.MailboxColumns.UNREAD_COUNT, 0);
            values.put(EmailContent.MailboxColumns.MESSAGE_COUNT, 0);
        }
        ...
        try {
            switch (match) {
                ...
                case QUICK_RESPONSE:
                    ...
                    switch(match) {
                        case MESSAGE:
                            final long mailboxId = values.getAsLong(EmailContent.Message.MAILBOX_KEY);
                            if (!uri.getBooleanQueryParameter(IS_UIPROVIDER, false)) {
                                notifyUIConversationMailbox(mailboxId);
                            }
                            notifyUIFolder(mailboxId, values.getAsLong(EmailContent.Message.ACCOUNT_KEY));
                            break;
                        case MAILBOX:
                            if (values.containsKey(EmailContent.MailboxColumns.TYPE)) {
                                if (values.getAsInteger(EmailContent.MailboxColumns.TYPE) <
                                        Mailbox.TYPE_NOT_EMAIL) {
                                    // Notify the account when a new mailbox is added
                                    final Long accountId =
                                            values.getAsLong(EmailContent.MailboxColumns.ACCOUNT_KEY);
                        ...
                        case ATTACHMENT:
                            int flags = 0;
                            if (values.containsKey(EmailContent.Attachment.FLAGS)) {
                                flags = values.getAsInteger(EmailContent.Attachment.FLAGS);
                            }
                            // Report all new attachments to the download service
                            if (TextUtils.isEmpty(values.getAsString(EmailContent.Attachment.LOCATION))) {
                ...
                case MAILBOX_ID:
                    ...
                    values.put(EmailContent.MessageColumns.MAILBOX_KEY, longId);
                    return insert(EmailContent.Message.CONTENT_URI, values); // Recurse
                case MESSAGE_ID:
                    // This implies adding an attachment to a message.
                    id = uri.getPathSegments().get(1);
                    longId = Long.parseLong(id);
                    values.put(EmailContent.AttachmentColumns.MESSAGE_KEY, longId);
                    return insert(EmailContent.Attachment.CONTENT_URI, values); // Recurse
                case ACCOUNT_ID:
                    // This implies adding a mailbox to an account.
                    longId = Long.parseLong(uri.getPathSegments().get(1));
                    values.put(EmailContent.MailboxColumns.ACCOUNT_KEY, longId);
                    return insert(Mailbox.CONTENT_URI, values); // Recurse
                case ATTACHMENTS_MESSAGE_ID:
                    longId = db.insert(TABLE_NAMES.valueAt(table), "foo", values);
                    resultUri = ContentUris.withAppendedId(EmailContent.Attachment.CONTENT_URI, longId);
                    break;
                default:
                    throw new IllegalArgumentException("Unknown URL " + uri);
            }
        ...
        try {
            switch (match) {
                ...
                case ATTACHMENTS_MESSAGE_ID:
                    // All attachments for the given message
                    id = uri.getPathSegments().get(2);
                    c = db.query(EmailContent.Attachment.TABLE_NAME, projection,
                            whereWith(EmailContent.Attachment.MESSAGE_KEY + "=" + id, selection),
                            selectionArgs, null, null, sortOrder, limit);
                    break;
                case QUICK_RESPONSE_ACCOUNT_ID:
    ...
    private static final String MESSAGE_CHANGE_LOG_TABLE_VALUES_PREFIX = ") values (%s, "
            + "(select " + EmailContent.Message.SERVER_ID + " from " + EmailContent.Message.TABLE_NAME + " where _id=%s),"
            + "(select " + EmailContent.Message.ACCOUNT_KEY + " from " + EmailContent.Message.TABLE_NAME + " where _id=%s),"
    ...
    private static final String MESSAGE_MOVE_INSERT = MESSAGE_CHANGE_LOG_TABLE_INSERT_PREFIX
            ...
            + "(select " + EmailContent.Message.MAILBOX_KEY + " from " + EmailContent.Message.TABLE_NAME + " where _id=%s),"
            + "%d,"
            + "(select " + Mailbox.SERVER_ID + " from " + Mailbox.TABLE_NAME + " where _id=(select "
            + EmailContent.Message.MAILBOX_KEY + " from " + EmailContent.Message.TABLE_NAME + " where _id=%s)),"
    ...
    private static final String MESSAGE_STATE_CHANGE_INSERT = MESSAGE_CHANGE_LOG_TABLE_INSERT_PREFIX
            ...
            + MESSAGE_CHANGE_LOG_TABLE_VALUES_PREFIX
            + "(select " + EmailContent.Message.FLAG_READ + " from " + EmailContent.Message.TABLE_NAME + " where _id=%s),"
            + "%d,"
            + "(select " + EmailContent.Message.FLAG_FAVORITE + " from " + EmailContent.Message.TABLE_NAME + " where _id=%s),"
            + "%d)";
    ...
    private static final String ACCOUNT_INTEGRITY_SQL =
            ...
            " where accountKey=? group by " + EmailContent.MailboxColumns.SERVER_ID + ") where dupes > 1";
    ...
    private static final String GET_MESSAGE_DETAILS = "SELECT"
            + " h." + EmailContent.HostAuthColumns.PROTOCOL + ","
            + " m." + EmailContent.Message.MAILBOX_KEY + ","
            + " a." + EmailContent.AccountColumns.ID
            + " FROM " + EmailContent.Message.TABLE_NAME + " AS m"
            + " INNER JOIN " + Account.TABLE_NAME + " AS a"
            + " ON m." + EmailContent.MessageColumns.ACCOUNT_KEY + "=a." + EmailContent.AccountColumns.ID
            + " INNER JOIN " + HostAuth.TABLE_NAME + " AS h"
            + " ON a." + EmailContent.AccountColumns.HOST_AUTH_KEY_RECV + "=h." + EmailContent.HostAuthColumns.ID
            + " WHERE m." + EmailContent.MessageColumns.ID + "=?";
    ...
    private static final String GET_ACCOUNT_DETAILS = "SELECT"
            + " h." + EmailContent.HostAuthColumns.PROTOCOL + ","
            + " a." + EmailContent.AccountColumns.EMAIL_ADDRESS + ","
            + " a." + EmailContent.AccountColumns.SYNC_KEY
            + " FROM " + Account.TABLE_NAME + " AS a"
            + " INNER JOIN " + HostAuth.TABLE_NAME + " AS h"
            + " ON a." + EmailContent.AccountColumns.HOST_AUTH_KEY_RECV + "=h." + EmailContent.HostAuthColumns.ID
            + " WHERE a." + EmailContent.AccountColumns.ID + "=?";
     ...
     * @param context A {@link android.content.Context}.
     * @param db The {@link android.database.sqlite.SQLiteDatabase}.
     * @param values The {@link android.content.ContentValues} that were updated for the mailbox.
    ...
    private static boolean restartPushForMailbox(final Context context, final SQLiteDatabase db,
        ...
        if (values.containsKey(EmailContent.MailboxColumns.SYNC_LOOKBACK) ||
                values.containsKey(EmailContent.MailboxColumns.SYNC_INTERVAL)) {
     ...
     * @param context A {@link android.content.Context}.
     * @param db The {@link android.database.sqlite.SQLiteDatabase}.
     * @param values The {@link android.content.ContentValues} that were updated for the account.
     * @param accountId The id of the account.
     * @return Whether or not the push was restarted.
     */
    private static boolean restartPushForAccount(final Context context, final SQLiteDatabase db,
            final ContentValues values, final String accountId) {
        if (values.containsKey(EmailContent.AccountColumns.SYNC_LOOKBACK) ||
                values.containsKey(EmailContent.AccountColumns.SYNC_INTERVAL)) {
            return restartPush(context, db, accountId);
        }
        return false;
    }
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // These columns are maintained via triggers
        if (match == MAILBOX_ID || match == MAILBOX) {
            values.remove(EmailContent.MailboxColumns.UNREAD_COUNT);
            values.remove(EmailContent.MailboxColumns.MESSAGE_COUNT);
        }
        ...
        try {
            switch (match) {
                ...
                case POLICY_ID:
                    ...
                    if (match == SYNCED_MESSAGE_ID) {
                        ...
                        if (isEas) {
                            ...
                            Long dstFolderId = values.getAsLong(EmailContent.MessageColumns.MAILBOX_KEY);
                            if (dstFolderId != null) {
                                addToMessageMove(db, id, dstFolderId);
                            }
                            Integer flagRead = values.getAsInteger(EmailContent.MessageColumns.FLAG_READ);
                            Integer flagFavorite = values.getAsInteger(EmailContent.MessageColumns.FLAG_FAVORITE);
                    ...
                    if (match == MESSAGE_ID || match == SYNCED_MESSAGE_ID) {
                        ...
                        if (values.containsKey(EmailContent.Attachment.FLAGS)) {
                            int flags = values.getAsInteger(EmailContent.Attachment.FLAGS);
                        ...
                        if (values.containsKey(EmailContent.AttachmentColumns.UI_STATE) ||
                                values.containsKey(EmailContent.AttachmentColumns.UI_DOWNLOADED_SIZE)) {
                            // Notify on individual attachment
                            notifyUI(UIPROVIDER_ATTACHMENT_NOTIFIER, id);
                            EmailContent.Attachment att = EmailContent.Attachment.restoreAttachmentWithId(context, attId);
                            if (att != null) {
                ...
                case BODY:
                    ...
                    if (result == 0 && selection.equals(EmailContent.Body.SELECTION_BY_MESSAGE_KEY)) {
                ...
                case POLICY:
                    if (match == ATTACHMENT) {
                        if (values.containsKey(EmailContent.AttachmentColumns.LOCATION) &&
                                TextUtils.isEmpty(values.getAsString(EmailContent.AttachmentColumns.LOCATION))) {
                            LogUtils.w(TAG, new Throwable(), "attachment with blank location");
                        }
                    }
                    result = db.update(tableName, values, selection, selectionArgs);
    ...
    private static Uri getBaseNotificationUri(int match) {
        ...
        switch (match) {
            ...
            case MESSAGE_ID:
            case SYNCED_MESSAGE_ID:
                baseUri = EmailContent.Message.NOTIFIER_URI;
                break;
            case ACCOUNT:
            case ACCOUNT_ID:
     ...
     * {@link android.database.ContentObserver}, observers of <<baseURI>> will receive multiple notifications.
     *
    ...
    private void sendNotifierChange(Uri baseUri, String op, String id) {
        ...
        if (baseUri.equals(EmailContent.Message.NOTIFIER_URI)) {
    ...
    private Cursor notificationQuery(final Uri uri) {
        ...
        sqlBuilder.append(EmailContent.MessageColumns.MAILBOX_KEY).append(", ");
        sqlBuilder.append("SUM(CASE ")
                .append(EmailContent.MessageColumns.FLAG_READ).append(" WHEN 0 THEN 1 ELSE 0 END), ");
        sqlBuilder.append("SUM(CASE ")
                .append(EmailContent.MessageColumns.FLAG_SEEN).append(" WHEN 0 THEN 1 ELSE 0 END)\n");
        sqlBuilder.append("FROM ");
        sqlBuilder.append(EmailContent.Message.TABLE_NAME).append('\n');
        sqlBuilder.append("WHERE ");
        sqlBuilder.append(EmailContent.MessageColumns.ACCOUNT_KEY).append(" = ?\n");
        sqlBuilder.append("GROUP BY ");
        sqlBuilder.append(EmailContent.MessageColumns.MAILBOX_KEY);
    ...
    private static final String CONVERSATION_FLAGS =
            "CASE WHEN (" + EmailContent.MessageColumns.FLAGS + "&" + EmailContent.Message.FLAG_INCOMING_MEETING_INVITE +
                ") !=0 THEN " + UIProvider.ConversationFlags.CALENDAR_INVITE +
                " ELSE 0 END + " +
            "CASE WHEN (" + EmailContent.MessageColumns.FLAGS + "&" + EmailContent.Message.FLAG_FORWARDED +
                ") !=0 THEN " + UIProvider.ConversationFlags.FORWARDED +
                " ELSE 0 END + " +
             "CASE WHEN (" + EmailContent.MessageColumns.FLAGS + "&" + EmailContent.Message.FLAG_REPLIED_TO +
    ...
    private static final String CONVERSATION_COLOR =
            "@CASE (" + EmailContent.MessageColumns.ACCOUNT_KEY + " - 1) % " + ACCOUNT_COLORS.length +
    ...
    private static final String ACCOUNT_COLOR =
            "@CASE (" + EmailContent.AccountColumns.ID + " - 1) % " + ACCOUNT_COLORS.length +
                    " WHEN 0 THEN " + ACCOUNT_COLORS[0] +
                    " WHEN 1 THEN " + ACCOUNT_COLORS[1] +
                    " WHEN 2 THEN " + ACCOUNT_COLORS[2] +
                    " WHEN 3 THEN " + ACCOUNT_COLORS[3] +
    ...
    private static ProjectionMap getMessageListMap() {
        if (sMessageListMap == null) {
            sMessageListMap = ProjectionMap.builder()
                .add(BaseColumns._ID, EmailContent.MessageColumns.ID)
                .add(ConversationColumns.URI, uriWithId("uimessage"))
                .add(ConversationColumns.MESSAGE_LIST_URI, uriWithId("uimessage"))
                .add(ConversationColumns.SUBJECT, EmailContent.MessageColumns.SUBJECT)
                .add(ConversationColumns.SNIPPET, EmailContent.MessageColumns.SNIPPET)
                .add(ConversationColumns.CONVERSATION_INFO, null)
                .add(ConversationColumns.DATE_RECEIVED_MS, EmailContent.MessageColumns.TIMESTAMP)
                .add(ConversationColumns.HAS_ATTACHMENTS, EmailContent.MessageColumns.FLAG_ATTACHMENT)
                .add(ConversationColumns.NUM_MESSAGES, "1")
                .add(ConversationColumns.NUM_DRAFTS, "0")
                .add(ConversationColumns.SENDING_STATE,
                        Integer.toString(ConversationSendingState.OTHER))
                .add(ConversationColumns.PRIORITY,
                        Integer.toString(ConversationPriority.LOW))
                .add(ConversationColumns.READ, EmailContent.MessageColumns.FLAG_READ)
                .add(ConversationColumns.SEEN, EmailContent.MessageColumns.FLAG_SEEN)
                .add(ConversationColumns.STARRED, EmailContent.MessageColumns.FLAG_FAVORITE)
                .add(ConversationColumns.FLAGS, CONVERSATION_FLAGS)
                .add(ConversationColumns.ACCOUNT_URI,
                        uriWithColumn("uiaccount", EmailContent.MessageColumns.ACCOUNT_KEY))
                .add(ConversationColumns.SENDER_INFO, EmailContent.MessageColumns.FROM_LIST)
                .build();
        }
    ...
    private static final String MESSAGE_DRAFT_TYPE =
        "CASE WHEN (" + EmailContent.MessageColumns.FLAGS + "&" + EmailContent.Message.FLAG_TYPE_ORIGINAL +
            ") !=0 THEN " + DraftType.COMPOSE +
        " WHEN (" + EmailContent.MessageColumns.FLAGS + "&" + EmailContent.Message.FLAG_TYPE_REPLY_ALL +
            ") !=0 THEN " + DraftType.REPLY_ALL +
        " WHEN (" + EmailContent.MessageColumns.FLAGS + "&" + EmailContent.Message.FLAG_TYPE_REPLY +
            ") !=0 THEN " + DraftType.REPLY +
        " WHEN (" + EmailContent.MessageColumns.FLAGS + "&" + EmailContent.Message.FLAG_TYPE_FORWARD +
    ...
    private static ProjectionMap getMessageViewMap() {
        if (sMessageViewMap == null) {
            sMessageViewMap = ProjectionMap.builder()
                .add(BaseColumns._ID, EmailContent.Message.TABLE_NAME + "." + EmailContent.MessageColumns.ID)
                .add(UIProvider.MessageColumns.SERVER_ID, EmailContent.SyncColumns.SERVER_ID)
                .add(UIProvider.MessageColumns.URI, uriWithFQId("uimessage", EmailContent.Message.TABLE_NAME))
                .add(UIProvider.MessageColumns.CONVERSATION_ID,
                        uriWithFQId("uimessage", EmailContent.Message.TABLE_NAME))
                ...
                .add(UIProvider.MessageColumns.BODY_HTML, EmailContent.Body.HTML_CONTENT)
                .add(UIProvider.MessageColumns.BODY_TEXT, EmailContent.Body.TEXT_CONTENT)
                ...
                .add(UIProvider.MessageColumns.ATTACHMENT_LIST_URI,
                        uriWithFQId("uiattachments", EmailContent.Message.TABLE_NAME))
    ...
    private static final String FOLDER_CAPABILITIES =
        "CASE WHEN (" + EmailContent.MailboxColumns.FLAGS + "&" + Mailbox.FLAG_ACCEPTS_MOVED_MAIL +
     ...
     */
    private static final String FOLDER_TYPE = "CASE " + EmailContent.MailboxColumns.TYPE
    ...
    private static final String FOLDER_ICON = "CASE " + EmailContent.MailboxColumns.TYPE
    ...
    private static final String TOTAL_COUNT = "CASE WHEN "
            + EmailContent.MailboxColumns.TOTAL_COUNT + "<0 OR "
            + EmailContent.MailboxColumns.TYPE + "=" + Mailbox.TYPE_DRAFTS + " OR "
            + EmailContent.MailboxColumns.TYPE + "=" + Mailbox.TYPE_OUTBOX + " OR "
            + EmailContent.MailboxColumns.TYPE + "=" + Mailbox.TYPE_TRASH
            + " THEN " + EmailContent.MailboxColumns.MESSAGE_COUNT
            + " ELSE " + EmailContent.MailboxColumns.TOTAL_COUNT + " END";

    private static ProjectionMap getFolderListMap() {
        if (sFolderListMap == null) {
            sFolderListMap = ProjectionMap.builder()
                .add(BaseColumns._ID, EmailContent.MailboxColumns.ID)
                .add(UIProvider.FolderColumns.PERSISTENT_ID, EmailContent.MailboxColumns.SERVER_ID)
                ...
                .add(UIProvider.FolderColumns.HAS_CHILDREN,
                        EmailContent.MailboxColumns.FLAGS + "&" + Mailbox.FLAG_HAS_CHILDREN)
                ...
                .add(UIProvider.FolderColumns.UNREAD_COUNT, EmailContent.MailboxColumns.UNREAD_COUNT)
                .add(UIProvider.FolderColumns.TOTAL_COUNT, TOTAL_COUNT)
                .add(UIProvider.FolderColumns.REFRESH_URI, uriWithId(QUERY_UIREFRESH))
                .add(UIProvider.FolderColumns.SYNC_STATUS, EmailContent.MailboxColumns.UI_SYNC_STATUS)
                .add(UIProvider.FolderColumns.LAST_SYNC_RESULT, EmailContent.MailboxColumns.UI_LAST_SYNC_RESULT)
                ...
                .add(UIProvider.FolderColumns.HIERARCHICAL_DESC, EmailContent.MailboxColumns.HIERARCHICAL_NAME)
                .add(UIProvider.FolderColumns.PARENT_URI, "case when " + EmailContent.MailboxColumns.PARENT_KEY
                        + "=" + Mailbox.NO_MAILBOX + " then NULL else " +
                        uriWithColumn("uifolder", EmailContent.MailboxColumns.PARENT_KEY) + " end")
                /**
                ...
                .add(UIProvider.FolderColumns.UNREAD_SENDERS,
                        "(SELECT group_concat(" + EmailContent.MessageColumns.FROM_LIST + ") FROM " +
                        "(SELECT " + EmailContent.MessageColumns.FROM_LIST + " FROM " + EmailContent.Message.TABLE_NAME +
                        " WHERE " + EmailContent.MessageColumns.MAILBOX_KEY + "=" + Mailbox.TABLE_NAME + "." +
                        EmailContent.MailboxColumns.ID + " AND " + EmailContent.MessageColumns.FLAG_READ + "=0" +
                        " GROUP BY " + EmailContent.MessageColumns.FROM_LIST + " ORDER BY " +
    ...
    private static ProjectionMap getAccountListMap(Context context) {
        if (sAccountListMap == null) {
            ...
            final ProjectionMap.Builder builder = ProjectionMap.builder()
                    .add(BaseColumns._ID, EmailContent.AccountColumns.ID)
                    ...
                    .add(UIProvider.AccountColumns.NAME, EmailContent.AccountColumns.DISPLAY_NAME)
                    ...
                    .add(UIProvider.AccountColumns.SENDER_NAME,
                            EmailContent.AccountColumns.SENDER_NAME)
                    ...
                    .add(UIProvider.AccountColumns.SettingsColumns.SIGNATURE,
                            EmailContent.AccountColumns.SIGNATURE)
                    ...
                    .add(UIProvider.AccountColumns.SettingsColumns.MAX_ATTACHMENT_SIZE,
                            EmailContent.AccountColumns.MAX_ATTACHMENT_SIZE)
     ...
     */
    private static final String MAILBOX_ORDER_BY = "CASE " + EmailContent.MailboxColumns.TYPE
        ...
        + " ELSE 10 END"
        + " ," + EmailContent.MailboxColumns.DISPLAY_NAME + " COLLATE LOCALIZED ASC";
    ...
    private static ProjectionMap getAttachmentMap() {
        if (sAttachmentMap == null) {
            sAttachmentMap = ProjectionMap.builder()
                .add(UIProvider.AttachmentColumns.NAME, EmailContent.AttachmentColumns.FILENAME)
                .add(UIProvider.AttachmentColumns.SIZE, EmailContent.AttachmentColumns.SIZE)
                .add(UIProvider.AttachmentColumns.URI, uriWithId("uiattachment"))
                .add(UIProvider.AttachmentColumns.CONTENT_TYPE, EmailContent.AttachmentColumns.MIME_TYPE)
                .add(UIProvider.AttachmentColumns.STATE, EmailContent.AttachmentColumns.UI_STATE)
                .add(UIProvider.AttachmentColumns.DESTINATION, EmailContent.AttachmentColumns.UI_DESTINATION)
                .add(UIProvider.AttachmentColumns.DOWNLOADED_SIZE,
                        EmailContent.AttachmentColumns.UI_DOWNLOADED_SIZE)
                .add(UIProvider.AttachmentColumns.CONTENT_URI, EmailContent.AttachmentColumns.CONTENT_URI)
                .add(UIProvider.AttachmentColumns.FLAGS, EmailContent.AttachmentColumns.FLAGS)
                .build();
        }
    ...
    private MessageQuery genQueryViewMessage(String[] uiProjection, String id) {
        ...
        EmailContent.Message msg = EmailContent.Message.restoreMessageWithId(context, messageId);
        ContentValues values = new ContentValues();
        String attachmentJson = null;
        if (msg != null) {
            EmailContent.Body body = EmailContent.Body.restoreBodyWithMessageId(context, messageId);
            if (body != null) {
            ...
            // Add attachments...
            EmailContent.Attachment[] atts = EmailContent.Attachment.restoreAttachmentsWithMessageId(context, messageId);
            if (atts.length > 0) {
                ArrayList<com.indeema.mail.providers.Attachment> uiAtts =
                        new ArrayList<com.indeema.mail.providers.Attachment>();
                for (EmailContent.Attachment att : atts) {
...
//                        continue;
//                    }
                    com.indeema.mail.providers.Attachment uiAtt =
                            new com.indeema.mail.providers.Attachment();
                ...
                attachmentJson = com.indeema.mail.providers.Attachment.toJSONArray(uiAtts);
            }
            if (msg.mDraftInfo != 0) {
                values.put(UIProvider.MessageColumns.APPEND_REF_MESSAGE_CONTENT,
                        (msg.mDraftInfo & EmailContent.Message.DRAFT_INFO_APPEND_REF_MESSAGE) != 0 ? 1 : 0);
                values.put(UIProvider.MessageColumns.QUOTE_START_POS,
                        msg.mDraftInfo & EmailContent.Message.DRAFT_INFO_QUOTE_POS_MASK);
            }
            if ((msg.mFlags & EmailContent.Message.FLAG_INCOMING_MEETING_INVITE) != 0) {
            ...
            final Uri attachmentListUri = uiUri("uiattachments", messageId).buildUpon()
                    .appendQueryParameter("MessageLoaded",
                            msg.mFlagLoaded == EmailContent.Message.FLAG_LOADED_COMPLETE ? "true" : "false")
                    .build();
        ...
        sb.append(" FROM " + EmailContent.Message.TABLE_NAME + " LEFT JOIN " + EmailContent.Body.TABLE_NAME + " ON " +
                EmailContent.Body.MESSAGE_KEY + "=" + EmailContent.Message.TABLE_NAME + "." + EmailContent.Message.RECORD_ID + " WHERE " +
                EmailContent.Message.TABLE_NAME + "." + EmailContent.Message.RECORD_ID + "=?");
    ...
    private static void appendConversationInfoColumns(final StringBuilder stringBuilder) {
        ...
        stringBuilder.append(',').append(EmailContent.MessageColumns.DISPLAY_NAME)
                .append(',').append(EmailContent.MessageColumns.FROM_LIST);
    }
    ...
    private static String genQueryMailboxMessages(String[] uiProjection, final boolean unseenOnly) {
        ...
        sb.append(" FROM " + EmailContent.Message.TABLE_NAME + " WHERE " +
                EmailContent.Message.FLAG_LOADED_SELECTION + " AND " +
                EmailContent.Message.MAILBOX_KEY + "=? ");
        if (unseenOnly) {
            sb.append("AND ").append(EmailContent.MessageColumns.FLAG_SEEN).append(" = 0 ");
            sb.append("AND ").append(EmailContent.MessageColumns.FLAG_READ).append(" = 0 ");
        }
        sb.append("ORDER BY " + EmailContent.MessageColumns.TIMESTAMP + " DESC ");
    ...
    private static Cursor getVirtualMailboxMessagesCursor(SQLiteDatabase db, String[] uiProjection,
        ...
        ContentValues values = new ContentValues();
        values.put(ConversationColumns.COLOR, CONVERSATION_COLOR);
        ...
        if (isCombinedMailbox(mailboxId)) {
            if (unseenOnly) {
                sb.append(EmailContent.MessageColumns.FLAG_SEEN).append("=0 AND ");
                sb.append(EmailContent.MessageColumns.FLAG_READ).append("=0 AND ");
            }
            selectionArgs = null;
        } else {
            ...
            sb.append(EmailContent.MessageColumns.ACCOUNT_KEY).append("=? AND ");
            selectionArgs = new String[]{getVirtualMailboxAccountIdString(mailboxId)};
        }
        switch (getVirtualMailboxType(mailboxId)) {
            case Mailbox.TYPE_INBOX:
                sb.append(EmailContent.MessageColumns.MAILBOX_KEY + " IN (SELECT " + EmailContent.MailboxColumns.ID +
                        " FROM " + Mailbox.TABLE_NAME + " WHERE " + EmailContent.MailboxColumns.TYPE +
                        "=" + Mailbox.TYPE_INBOX + ")");
                break;
            case Mailbox.TYPE_STARRED:
                sb.append(EmailContent.MessageColumns.FLAG_FAVORITE + "=1");
                break;
            case Mailbox.TYPE_UNREAD:
                sb.append(EmailContent.MessageColumns.FLAG_READ + "=0 AND " + EmailContent.MessageColumns.MAILBOX_KEY +
                        " NOT IN (SELECT " + EmailContent.MailboxColumns.ID + " FROM " + Mailbox.TABLE_NAME +
                        " WHERE " + EmailContent.MailboxColumns.TYPE + "=" + Mailbox.TYPE_TRASH + ")");
                break;
            default:
                throw new IllegalArgumentException("No virtual mailbox for: " + mailboxId);
        }
    ...
    private static String genQueryConversation(String[] uiProjection) {
        StringBuilder sb = genSelect(getMessageListMap(), uiProjection);
        sb.append(" FROM " + EmailContent.Message.TABLE_NAME + " WHERE " + EmailContent.Message.RECORD_ID + "=?");
        return sb.toString();
    }
    ...
    private static String genQueryAccountMailboxes(String[] uiProjection) {
        ...
        sb.append(" FROM " + Mailbox.TABLE_NAME + " WHERE " + EmailContent.MailboxColumns.ACCOUNT_KEY +
                "=? AND " + EmailContent.MailboxColumns.TYPE + " < " + Mailbox.TYPE_NOT_EMAIL +
                " AND " + EmailContent.MailboxColumns.TYPE + " != " + Mailbox.TYPE_SEARCH +
                " AND " + EmailContent.MailboxColumns.PARENT_KEY + " < 0 ORDER BY ");
    ...
    private static String genQueryAccountAllMailboxes(String[] uiProjection) {
        ...
        sb.append(", case when " + EmailContent.MailboxColumns.HIERARCHICAL_NAME + " is null then " +
                EmailContent.MailboxColumns.DISPLAY_NAME + " else " + EmailContent.MailboxColumns.HIERARCHICAL_NAME +
                " end as h_name");
    ...
    private static String genQueryRecentMailboxes(String[] uiProjection) {
        ...
        sb.append(" FROM " + Mailbox.TABLE_NAME + " WHERE " + EmailContent.MailboxColumns.ACCOUNT_KEY +
                "=? AND " + EmailContent.MailboxColumns.TYPE + " < " + Mailbox.TYPE_NOT_EMAIL +
                " AND " + EmailContent.MailboxColumns.TYPE + " != " + Mailbox.TYPE_SEARCH +
                " AND " + EmailContent.MailboxColumns.PARENT_KEY + " < 0 AND " +
                EmailContent.MailboxColumns.LAST_TOUCHED_TIME + " > 0 ORDER BY " +
                EmailContent.MailboxColumns.LAST_TOUCHED_TIME + " DESC");
        return sb.toString();
    }

    private int getFolderCapabilities(EmailServiceUtils.EmailServiceInfo info, int type, long mailboxId) {
    ...
    private String genQueryMailbox(String[] uiProjection, String id) {
        ...
        if (mSearchParams != null && mailboxId == mSearchParams.mSearchMailboxId) {
            ...
            if (mailbox != null) {
                ...
                EmailServiceUtils.EmailServiceInfo info = EmailServiceUtils.getServiceInfo(context, protocol);
        ...
        sb.append(" FROM " + Mailbox.TABLE_NAME + " WHERE " + EmailContent.MailboxColumns.ID + "=?");
        return sb.toString();
    }
    ...
    private String genQueryAccount(String[] uiProjection, String id) {
        ...
        EmailServiceUtils.EmailServiceInfo info = null;
        ...
        sb.append(" FROM " + Account.TABLE_NAME + " WHERE " + EmailContent.AccountColumns.ID + "=?");
        return sb.toString();
    }
        ...
        switch (mailboxType) {
            case Mailbox.TYPE_INBOX:
                if (accountId == COMBINED_ACCOUNT_ID) {
                    ...
                    final int unreadCount = EmailContent.count(getContext(), EmailContent.Message.CONTENT_URI,
                            EmailContent.MessageColumns.MAILBOX_KEY + " IN (SELECT " + EmailContent.MailboxColumns.ID
                            + " FROM " + Mailbox.TABLE_NAME + " WHERE " + EmailContent.MailboxColumns.TYPE
                            + "=" + Mailbox.TYPE_INBOX + ") AND " + EmailContent.MessageColumns.FLAG_READ + "=0",
                            null);
            ...
            case Mailbox.TYPE_UNREAD: {
                ...
                if (accountId == COMBINED_ACCOUNT_ID) {
                    ...
                    accountKeyClause = EmailContent.MessageColumns.ACCOUNT_KEY + "= ? AND ";
                    whereArgs = new String[] { accountIdString };
                }
                final int unreadCount = EmailContent.count(getContext(), EmailContent.Message.CONTENT_URI,
                        accountKeyClause + EmailContent.MessageColumns.FLAG_READ + "=0 AND "
                        + EmailContent.MessageColumns.MAILBOX_KEY + " NOT IN (SELECT " + EmailContent.MailboxColumns.ID
                        + " FROM " + Mailbox.TABLE_NAME + " WHERE " + EmailContent.MailboxColumns.TYPE + "="
                ...
                final int starredCount = EmailContent.count(getContext(), EmailContent.Message.CONTENT_URI,
                        accountKeyClause + EmailContent.MessageColumns.FLAG_FAVORITE + "=1", whereArgs);
    ...
    private static String genQueryAttachments(String[] uiProjection,
        ...
        sb.append(" FROM ")
                .append(EmailContent.Attachment.TABLE_NAME)
                .append(" WHERE ")
                .append(EmailContent.AttachmentColumns.MESSAGE_KEY)
                .append(" =? ");
        ...
        if (contentTypeQueryParameters != null && !contentTypeQueryParameters.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                sb.append(EmailContent.AttachmentColumns.MIME_TYPE)
                        .append(" LIKE '")
                        .append(contentType)
                        .append("%'");
    ...
    private String genQueryAttachment(String[] uiProjection, String idString) {
        ...
        EmailContent.Attachment att = EmailContent.Attachment.restoreAttachmentWithId(getContext(), id);
        // MAKE SURE THESE VALUES STAY IN SYNC WITH GEN QUERY ATTACHMENTS
        ContentValues values = new ContentValues(2);
        values.put(EmailContent.AttachmentColumns.CONTENT_URI,
        ...
        sb.append(" FROM ")
                .append(EmailContent.Attachment.TABLE_NAME)
                .append(" WHERE ")
                .append(EmailContent.AttachmentColumns.ID)
                .append(" =? ");
        return sb.toString();
    }
    ...
    private static String genQuerySubfolders(String[] uiProjection) {
        StringBuilder sb = genSelect(getFolderListMap(), uiProjection);
        sb.append(" FROM " + Mailbox.TABLE_NAME + " WHERE " + EmailContent.MailboxColumns.PARENT_KEY +
                " =? ORDER BY ");
        sb.append(MAILBOX_ORDER_BY);
        return sb.toString();
    }
    ...
    private Uri[] defaultRecentFolders(final String id) {
        ...
        sb.append(" FROM ")
                ...
                .append(" WHERE ")
                .append(EmailContent.MailboxColumns.ACCOUNT_KEY)
                .append(" = ")
                .append(id)
                .append(" AND ")
                .append(EmailContent.MailboxColumns.TYPE)
                .append(" IN (")
    ...
    /**
     * Convenience method to create a {@link com.indeema.mail.providers.Folder}
     * @param context to get a {@link android.content.ContentResolver}
     * @param mailboxId id of the {@link com.indeema.emailcommon.provider.Mailbox} that we want
     * @return the {@link com.indeema.mail.providers.Folder} or null
     */
    ...
    static class AttachmentsCursor extends CursorWrapper {
        ...
        public String getString(int column) {
            if (column == mContentUriIndex) {
                ...
                final EmailContent.Attachment att = EmailContent.Attachment.restoreAttachmentWithId(mContext, id);
                if (att == null) return "";
     ...
     * in the supplied {@link android.database.MatrixCursor} in the format required for {@link com.indeema.mail.providers.Folder}.
     * As a convenience, the modified {@link android.database.MatrixCursor} is also returned.
     * @param mc the {@link android.database.MatrixCursor} into which the mailbox data will be converted
     ...
     * @return the {@link android.database.MatrixCursor} containing the transformed data.
     */
     ...
     * @param inputCursor A cursor containing all columns of {@link com.indeema.mail.providers.UIProvider.FolderColumns}.
     ...
     * to the {@link com.indeema.mail.providers.UIProvider.FolderType} requested.
     * @param folderType {@link com.indeema.mail.providers.UIProvider.FolderType} value for the folder
     * @param defaultName a {@link String} to use in case the {@link com.indeema.mail.providers.UIProvider.FolderType}
    ...
    /**
     * Converts a {@link com.indeema.emailcommon.provider.Mailbox} type value to its {@link com.indeema.mail.providers.UIProvider.FolderType}
     * equivalent.
     * @param mailboxType a {@link com.indeema.emailcommon.provider.Mailbox} type
     * @return a {@link com.indeema.mail.providers.UIProvider.FolderType} value
     */
    ...
    // be removed.
    private static EmailContent.Attachment convertUiAttachmentToAttachment(
            com.indeema.mail.providers.Attachment uiAtt, String cachedFile, long accountKey) {
        final EmailContent.Attachment att = new EmailContent.Attachment();
        ...
        if (!TextUtils.isEmpty(cachedFile)) {
            ...
            cachedFileBuilder.appendQueryParameter(EmailContent.Attachment.CACHED_FILE_QUERY_PARAM, cachedFile);
     ...
     */
    private Uri uiSaveMessage(EmailContent.Message msg, Mailbox mailbox, Bundle extras) {
        final Context context = getContext();
        // Fill in the message
        ...
        switch(draftType) {
            case DraftType.FORWARD:
                flags |= EmailContent.Message.FLAG_TYPE_FORWARD;
                break;
            case DraftType.REPLY_ALL:
                flags |= EmailContent.Message.FLAG_TYPE_REPLY_ALL;
                //$FALL-THROUGH$
            case DraftType.REPLY:
                flags |= EmailContent.Message.FLAG_TYPE_REPLY;
                break;
            case DraftType.COMPOSE:
                flags |= EmailContent.Message.FLAG_TYPE_ORIGINAL;
                break;
        }
        int draftInfo = 0;
        if (extras.containsKey(UIProvider.MessageColumns.QUOTE_START_POS)) {
            ...
            if (extras.getInt(UIProvider.MessageColumns.APPEND_REF_MESSAGE_CONTENT) != 0) {
                draftInfo |= EmailContent.Message.DRAFT_INFO_APPEND_REF_MESSAGE;
            }
        }
        if (!extras.containsKey(UIProvider.MessageColumns.APPEND_REF_MESSAGE_CONTENT)) {
            flags |= EmailContent.Message.FLAG_NOT_INCLUDE_QUOTED_TEXT;
        }
        ...
        final List<com.indeema.mail.providers.Attachment> uiAtts =
                com.indeema.mail.providers.Attachment.fromJSONArray(
                        extras.getString(UIProvider.MessageColumns.ATTACHMENTS));
        final ArrayList<EmailContent.Attachment> atts = new ArrayList<EmailContent.Attachment>();
        ...
        for (com.indeema.mail.providers.Attachment uiAtt: uiAtts) {
            ...
            if (attUri != null && attUri.getAuthority().equals(EmailContent.AUTHORITY)) {
                ...
                final EmailContent.Attachment att = EmailContent.Attachment.restoreAttachmentWithId(context, attId);
                if (att != null) {
                    ...
                    p.setDataPosition(0);
                    final EmailContent.Attachment attClone = new EmailContent.Attachment(p);
                    p.recycle();
                    ...
                    if (mailbox.mType == Mailbox.TYPE_OUTBOX && att.getContentUri() == null &&
                        ...
                        attClone.mFlags |= EmailContent.Attachment.FLAG_DOWNLOAD_FORWARD;
        ...
        if (!msg.isSaved()) {
            ...
            ops.add(ContentProviderOperation.newDelete(
                    ContentUris.withAppendedId(EmailContent.Attachment.MESSAGE_ID_URI, msg.mId))
                    .build());
            // Delete the body
            ops.add(ContentProviderOperation.newDelete(EmailContent.Body.CONTENT_URI)
                    .withSelection(EmailContent.Body.MESSAGE_KEY + "=?", new String[] {Long.toString(msg.mId)})
                    .build());
        ...
        if (mailbox.mType == Mailbox.TYPE_OUTBOX) {
            ...
            if (originalMsgId != 0) {
                final EmailContent.Message originalMsg = EmailContent.Message.restoreMessageWithId(context, originalMsgId);
                ...
                if (originalMsg != null) {
                    ...
                    cv.put(EmailContent.Message.FLAGS, flags);
                    context.getContentResolver().update(ContentUris.withAppendedId(
                            EmailContent.Message.CONTENT_URI, originalMsgId), cv, null, null);
                }
            }
        }
    ...
    private Uri uiSaveDraftMessage(final long accountId, final Bundle extras) {
        ...
        if (extras.containsKey(BaseColumns._ID)) {
            ...
            msg = EmailContent.Message.restoreMessageWithId(getContext(), messageId);
        } else {
    ...
    private static int updateTimestamp(final Context context, String id, Uri[] folders){
        ...
        for (final Uri folder : folders) {
            touchValues.put(EmailContent.MailboxColumns.LAST_TOUCHED_TIME, now);
    ...
    private int uiUpdateAttachment(Uri uri, ContentValues uiValues) {
        ...
        if (stateValue != null) {
            ...
            EmailContent.Attachment attachment =
                    EmailContent.Attachment.restoreAttachmentWithId(context, attachmentId);
            if (attachment == null) {
                // Went away; ah, well...
                return result;
            }
            ...
            if (state == UIProvider.AttachmentState.NOT_SAVED
                ...
                values.put(EmailContent.AttachmentColumns.UI_STATE, UIProvider.AttachmentState.NOT_SAVED);
                values.put(EmailContent.AttachmentColumns.FLAGS,
                        attachment.mFlags &= ~EmailContent.Attachment.FLAG_DOWNLOAD_USER_REQUEST);
            ...
            if (state == UIProvider.AttachmentState.DOWNLOADING
                ...
                values.put(EmailContent.AttachmentColumns.UI_STATE, UIProvider.AttachmentState.DOWNLOADING);
                Integer destinationValue =
                        uiValues.getAsInteger(UIProvider.AttachmentColumns.DESTINATION);
                values.put(EmailContent.AttachmentColumns.UI_DESTINATION,
    ...
    private int uiUpdateFolder(final Context context, Uri uri, ContentValues uiValues) {
        ...
        if (uiValues.containsKey(ConversationColumns.SEEN)) {
            final int seenValue = uiValues.getAsInteger(ConversationColumns.SEEN);
        ...
        for (String columnName: uiValues.keySet()) {
            if (columnName.equals(EmailContent.MailboxColumns.LAST_TOUCHED_TIME)) {
                ourValues.put(EmailContent.MailboxColumns.LAST_TOUCHED_TIME, uiValues.getAsLong(columnName));
            }
        }
    ...
    private int markAllSeen(final Context context, final String mailboxId) {
        final SQLiteDatabase db = getDatabase(context);
        final String table = EmailContent.Message.TABLE_NAME;
        final ContentValues values = new ContentValues(1);
        values.put(EmailContent.MessageColumns.FLAG_SEEN, 1);
        final String whereClause = EmailContent.MessageColumns.MAILBOX_KEY + " = ?";
        final String[] whereArgs = new String[] {mailboxId};

        return db.update(table, values, whereClause, whereArgs);
    }

    private ContentValues convertUiMessageValues(EmailContent.Message message, ContentValues values) {
        final ContentValues ourValues = new ContentValues();
        for (String columnName : values.keySet()) {
            final Object val = values.get(columnName);
            if (columnName.equals(ConversationColumns.STARRED)) {
                putIntegerLongOrBoolean(ourValues, EmailContent.MessageColumns.FLAG_FAVORITE, val);
            } else if (columnName.equals(ConversationColumns.READ)) {
                putIntegerLongOrBoolean(ourValues, EmailContent.MessageColumns.FLAG_READ, val);
            } else if (columnName.equals(ConversationColumns.SEEN)) {
                putIntegerLongOrBoolean(ourValues, EmailContent.MessageColumns.FLAG_SEEN, val);
            } else if (columnName.equals(EmailContent.MessageColumns.MAILBOX_KEY)) {
                putIntegerLongOrBoolean(ourValues, EmailContent.MessageColumns.MAILBOX_KEY, val);
            ...
            } else if (columnName.equals(ConversationColumns.RAW_FOLDERS)) {
                ...
                if (flist.folders.size() != 1) {
                    ...
                    putIntegerLongOrBoolean(ourValues, EmailContent.MessageColumns.MAILBOX_KEY, mailboxId);
                }
            ...
            } else if (columnName.equals(ConversationColumns.VIEWED) ||
                    columnName.equals(UIProvider.ConversationOperations.Parameters.SUPPRESS_UNDO)) {
                // Ignore for now
            } else if (ConversationColumns.CONVERSATION_INFO.equals(columnName)) {
    ...
    private EmailContent.Message getMessageFromLastSegment(Uri uri) {
        ...
        return EmailContent.Message.restoreMessageWithId(getContext(), messageId);
    }
    ...
    private int uiUpdateMessage(Uri uri, ContentValues values, boolean forceSync) {
        Context context = getContext();
        EmailContent.Message msg = getMessageFromLastSegment(uri);
        if (msg == null) return 0;
        Mailbox mailbox = Mailbox.restoreMailboxWithId(context, msg.mMailboxKey);
        if (mailbox == null) return 0;
        Uri ourBaseUri =
                (forceSync || uploadsToServer(context, mailbox)) ? EmailContent.Message.SYNCED_CONTENT_URI :
                    EmailContent.Message.CONTENT_URI;
        ...
        for (String columnName: ourValues.keySet()) {
            if (columnName.equals(EmailContent.MessageColumns.MAILBOX_KEY)) {
                undoValues.put(EmailContent.MessageColumns.MAILBOX_KEY, msg.mMailboxKey);
            } else if (columnName.equals(EmailContent.MessageColumns.FLAG_READ)) {
                undoValues.put(EmailContent.MessageColumns.FLAG_READ, msg.mFlagRead);
            } else if (columnName.equals(EmailContent.MessageColumns.FLAG_SEEN)) {
                undoValues.put(EmailContent.MessageColumns.FLAG_SEEN, msg.mFlagSeen);
            } else if (columnName.equals(EmailContent.MessageColumns.FLAG_FAVORITE)) {
                undoValues.put(EmailContent.MessageColumns.FLAG_FAVORITE, msg.mFlagFavorite);
            }
        }
    ...
    private static final String[] MESSAGE_KEYS_PROJECTION =
            { EmailContent.MessageColumns.MAILBOX_KEY, EmailContent.MessageColumns.ACCOUNT_KEY };
     ...
     * @param uri The {@link android.net.Uri} for this message update.
     * @param messageId The id of the message that's been updated.
     * @param values The {@link android.content.ContentValues} that were updated in the message.
     */
    private void handleMessageUpdateNotifications(final Uri uri, final String messageId,
        ...
        if (values.containsKey(EmailContent.MessageColumns.FLAG_READ) ||
                values.containsKey(EmailContent.MessageColumns.MAILBOX_KEY)) {
            final Cursor c = query(
                    EmailContent.Message.CONTENT_URI.buildUpon().appendEncodedPath(messageId).build(),
    ...
    private int uiDeleteMessage(Uri uri) {
        ...
        if (mailbox.mType == Mailbox.TYPE_TRASH || mailbox.mType == Mailbox.TYPE_DRAFTS) {
            ...
            final int r = context.getContentResolver().delete(
                    ContentUris.withAppendedId(EmailContent.Message.SYNCED_CONTENT_URI, msg.mId), null, null);
        ...
        ContentValues values = new ContentValues();
        values.put(EmailContent.MessageColumns.MAILBOX_KEY, trashMailbox.mId);
    ...
    private int pickFolder(Uri uri, int type, int headerId) {
        ...
        try {
            if (ac.moveToFirst()) {
                final com.indeema.mail.providers.Account uiAccount =
                        new com.indeema.mail.providers.Account(ac);
        ...
        if (!mLastSequenceOps.isEmpty()) {
            try {
                ...
                MatrixCursor c = new MatrixCursorWithCachedColumns(
                        new String[] {ConversationColumns.URI},
    ...
    private void notifyUIConversation(Uri uri) {
        ...
        EmailContent.Message msg = EmailContent.Message.restoreMessageWithId(getContext(), Long.parseLong(id));
        if (msg != null) {
    ...
    private void updateAccountSyncInterval(final long accountId, final ContentValues values) {
        final Integer syncInterval = values.getAsInteger(EmailContent.AccountColumns.SYNC_INTERVAL);
     ...
     *                  {@link com.indeema.emailcommon.provider.Mailbox}).
    ...
    /**
     * Request a sync.
     * @param mailbox The {@link com.indeema.emailcommon.provider.Mailbox} we want to sync.
     * @param deltaMessageCount If we're requesting a load more, the number of additional messages
     *                          to sync.
     */
            ...
            public Void doInBackground(Void... params) {
                ...
                if (service != null) {
                    try {
                        ...
                        cv.put(EmailContent.MailboxColumns.TOTAL_COUNT, totalCount);
    ...
    private Cursor uiSearch(Uri uri, String[] projection) {
        ...
        if (mSearchParams.mOffset == 0) {
            ...
            ContentResolver resolver = context.getContentResolver();
            resolver.delete(EmailContent.Message.CONTENT_URI, EmailContent.Message.MAILBOX_KEY + "=" + searchMailboxId,
                    null);
            final ContentValues cv = new ContentValues(1);
    ...
    private static final String MAILBOXES_FOR_ACCOUNT_SELECTION = EmailContent.MailboxColumns.ACCOUNT_KEY + "=?";
    ...
    private static final String[] ACCOUNT_EMAIL_PROJECTION = { EmailContent.AccountColumns.EMAIL_ADDRESS };
    ...
    public void dump(FileDescriptor fd, PrintWriter writer, String[] args) {
        ...
        for (EmailServiceUtils.EmailServiceInfo info: EmailServiceUtils.getServiceInfoList(context)) {
            writer.println("  " + info);
        }
        writer.println();
