...
public class WidgetProvider extends BaseWidgetProvider {
    ...
    @Override
    protected com.indeema.mail.providers.Account getAccountObject(
    ...
    @Override
    protected boolean isAccountValid(Context context, com.indeema.mail.providers.Account account) {
        if (account != null) {
            final ContentResolver resolver = context.getContentResolver();
    ...
    protected void migrateLegacyWidgetInformation(Context context, int widgetId) {
        ...
        final com.indeema.mail.providers.Account uiAccount = getAccount(context, accountId);
    ...
    private static com.indeema.mail.providers.Account getAccount(Context context, long accountId) {
        final ContentResolver resolver = context.getContentResolver();
        final Cursor ac = resolver.query(EmailProvider.uiUri("uiaccount", accountId),
                UIProvider.ACCOUNTS_PROJECTION_NO_CAPABILITIES, null, null, null);

        com.indeema.mail.providers.Account uiAccount = getPopulatedAccountObject(ac);

        return uiAccount;
    }

    private static com.indeema.mail.providers.Account getPopulatedAccountObject(
        ...
        com.indeema.mail.providers.Account uiAccount = null;
        try {
     ...
     * {@link com.indeema.emailcommon.provider.Account#NO_ACCOUNT} if
     ...
     * {@link com.indeema.emailcommon.provider.Mailbox#NO_MAILBOX} if
