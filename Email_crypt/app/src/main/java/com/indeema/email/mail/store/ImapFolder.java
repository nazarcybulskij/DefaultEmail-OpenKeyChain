...
import com.indeema.emailcommon.internet.BinaryTempFileBody;
...
import com.indeema.emailcommon.utility.CountingOutputStream;
import com.indeema.emailcommon.utility.EOLConvertingOutputStream;
...
class ImapFolder extends Folder {
        ...
        try {
            try {
                ...
                LogUtils.d(Logging.LOG_TAG, "searchForUids '" + searchCriteria + "' results: " +
                        result.length);
                return result;
            } catch (ImapStore.ImapException me) {
    ...
    public Message getMessage(String uid) throws MessagingException {
        ...
        for (int i = 0; i < uids.length; i++) {
            if (uids[i].equals(uid)) {
                return new ImapStore.ImapMessage(uid, this);
            }
        }
        return null;
    }
    ...
    public Message[] getMessagesInternal(String[] uids, MessageRetrievalListener listener) {
        ...
        for (int i = 0; i < uids.length; i++) {
            ...
            final ImapStore.ImapMessage message = new ImapStore.ImapMessage(uid, this);
        ...
        try {
            ...
            do {
                ...
                try {
                    ...
                    ImapStore.ImapMessage message = (ImapStore.ImapMessage) messageMap.get(uid);
        ...
        if (bs.getElementOrNone(0).isList()) {
            ...
            if (size > 0) {
                if (part instanceof ImapStore.ImapMessage) {
                    ((ImapStore.ImapMessage) part).setSize(size);
